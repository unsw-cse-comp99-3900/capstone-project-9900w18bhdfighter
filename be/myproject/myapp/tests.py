...
class UserProfileTests(TestCase):
    ...
    def test_get_user_profile(self):
        response = self.client.get(self.userprofile_url, HTTP_AUTHORIZATION='Bearer ' + self.adm_token)
        self.assertEqual(response.status_code, 200)
    ...
    def test_put_user_profile(self):
        data = {
            'UserInformation': 'Hi, i am stu.'
        }

        response = self.client.put(self.userprofile_url, data=json.dumps(data), content_type='application/json',
    ...
    def test_change_user_passwd(self):
        ...
        response_passwd = self.client.put(self.user_passwd, data=json.dumps(data_passwd), content_type='application/json',
                                   HTTP_AUTHORIZATION='Bearer ' + self.adm_token)
    ...
    def test_create_deleted_user(self):
        ...
        response = self.client.post(self.create_user_url, data=json.dumps(data), content_type='application/json',
                                    HTTP_AUTHORIZATION='Bearer ' + self.adm_token)
        ...
        response_before_del = self.client.get(f'{self.create_user_url}/{user_id}', HTTP_AUTHORIZATION='Bearer ' + self.adm_token)
        self.assertEqual(response_before_del.status_code, 200)


        response_del = self.client.delete(f'{self.create_user_url}/{user_id}', content_type='application/json',
                                    HTTP_AUTHORIZATION='Bearer ' + self.adm_token)
        self.assertEqual(response_del.status_code, 204) #no content

        response_after_del = self.client.get(f'{self.create_user_url}/{user_id}', HTTP_AUTHORIZATION='Bearer ' + self.adm_token)
        self.assertEqual(response_after_del.status_code, 404)

    def test_update_user_area(self):
        response = self.client.get(self.all_area_url, HTTP_AUTHORIZATION='Bearer ' + self.adm_token)
        self.assertEqual(response.status_code, 200)
    ...
    def test_update_user_nonexist_areas(self):
        ...
        bad_response = self.client.put(self.userprofile_url, data=json.dumps(bad_data), content_type='application/json',
                                   HTTP_AUTHORIZATION='Bearer ' + self.adm_token)
        self.assertEqual(bad_response.status_code, 400)
        bad_response_data = bad_response.json()
        self.assertIn('errors', bad_response_data)
        self.assertEqual(bad_response_data['errors'], 'Area with ID -1 does not exist.\n')
    ...
    def test_update_user_exist_areas(self):
        ...
        self.client.put(self.userprofile_url, data=json.dumps(data), content_type='application/json',
                                   HTTP_AUTHORIZATION='Bearer ' + self.adm_token)
        ...
        self.assertEqual(response.status_code, 200)
        response_data = response.json()['data']
        self.assertEqual(response_data['Areas'][0], {'AreaID': 4, 'AreaName': 'Frameworks & Libraries'})
        self.assertEqual(response_data['Areas'][1], {'AreaID': 7, 'AreaName': 'Data Processing & Visualization'})
