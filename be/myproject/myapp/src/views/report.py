...
def generate_report(request):
    if request.method == "POST":
        ...
        if user.UserRole not in [3, 4, 5]:
            return JsonResponse(
                {"error": "Permission denied, you can not generate reports."},
                status=403,
            )
        ...
        report_url = request.build_absolute_uri(
            f"/media/reports/{os.path.basename(report_path)}"
        )
    return JsonResponse(
        {"message": "Report generated successfully", "report_url": report_url},
        status=201,
    )
...
def download_report(request):
    ...
    if user.UserRole not in [3, 4, 5]:
        return JsonResponse(
            {"error": "Permission denied, you can not generate reports."}, status=403
        )
