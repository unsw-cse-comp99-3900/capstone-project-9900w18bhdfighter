...
from myapp.src.models.models import Group, GroupMessage, Message, User
...
class Consumer(AsyncWebsocketConsumer):
    ...
    def save_user_message(self, data: Msg, isRead):
        ...
        msg = Message.objects.create(
            Content=content, Sender=sender, Receiver=receiver, IsRead=isRead
        )
        ...
        if receiver_channel_name in self.channel_layer.groups:
            ...
            await self.channel_layer.group_send(
                receiver_channel_name,
                {
                    ...
                    "message": WSMsgRspDTO(
                        200, "Message delivered", res, "user"
                    ).to_dict(),
                },
            )
            ...
            if receiver_channel_name in self.channel_layer.groups:
                await self.channel_layer.group_send(
                    receiver_channel_name,
                    {
                        "type": "receive_from_others",
                        "message": WSMsgRspDTO(
                            200, "Message delivered", res, "group"
                        ).to_dict(),
                    },
                )
        # inform sender that the message has been sent
